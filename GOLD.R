...
# =============================================================================
# Chi2 — faithful fast rewrite matching discretization::chi2() exactly
# Liu & Setiono, AAAI 1995
# =============================================================================

# ── Fast multivariate inconsistency rate — replicates incon() exactly ─────────
fast_incon <- function(data) {
  n    <- nrow(data)
  p    <- ncol(data)
  feat <- data[, seq_len(p - 1L), drop = FALSE]
  y    <- as.numeric(data[[p]])

  # all rows unique → no inconsistency
  if (!anyDuplicated(feat)) return(0)

  # group rows by identical feature pattern
  keys   <- do.call(paste, c(lapply(feat, as.character), sep = "\r"))
  groups <- split(y, keys)

  # inconsistency per group = group_size - max_class_count
  incon_count <- sum(vapply(groups, function(g) {
    if (length(g) <= 1L) return(0L)
    length(g) - max(tabulate(as.integer(as.factor(g))))
  }, integer(1)))

  incon_count / n
}

# ── Fast chiSq for a 2-row matrix — replicates chiSq() exactly ───────────────
fast_chi_sq <- function(d) {
  # d is a 2 x K matrix
  rs <- rowSums(d);  cs <- colSums(d);  n <- sum(d)
  if (n == 0) return(0)
  E  <- outer(rs, cs) / n
  ok <- E > 0
  sum((d[ok] - E[ok])^2 / E[ok])
}

# ── Fast value() — single-feature chiM, replicates value() exactly ───────────
# Returns list(cuts, disc) where:
#   cuts = interior cutpoints only (no min/max) — matches package exactly
#   disc = full dataset with column i updated
fast_value <- function(i, data, alpha) {
  p1        <- ncol(data)
  x         <- data[[i]]
  y         <- data[[p1]]
  K         <- length(unique(y))
  threshold <- qchisq(1 - alpha, df = max(K - 1L, 1L))

  z <- sort(unique(x))
  if (length(z) <= 1L)
    return(list(cuts = "", disc = data))

  # replicates value() exactly:
  # cutpoint = midpoints between adjacent unique values
  # midpoint = c(z[1], cutpoint, z[lenz])  ← actual min/max as boundaries
  dff      <- diff(z) / 2
  lenz     <- length(z)
  cutpoint <- z[seq_len(lenz - 1L)] + dff
  midpoint <- c(z[1L], cutpoint, z[lenz])

  # build frequency table using midpoint breaks — matches b = table(a, data[,p1])
  a <- cut(x, breaks = midpoint, include.lowest = TRUE)
  b <- as.array(table(a, y))

  # chiMerge loop — replicates value() repeat loop exactly
  repeat {
    m <- nrow(b)
    if (length(dim(b)) < 2L || m < 2L) break

    # chi-square for each adjacent pair
    test <- vapply(seq_len(m - 1L), function(k)
      fast_chi_sq(b[c(k, k + 1L), , drop = FALSE]), numeric(1))

    k <- which.min(test)
    if (test[k] > threshold) break

    # merge row k+1 into k
    b[k + 1L, ] <- b[k, ] + b[k + 1L, ]
    cutpoint    <- cutpoint[-k]
    midpoint    <- midpoint[-(k + 1L)]
    b           <- b[-(k), , drop = FALSE]
  }

  disc       <- data
  disc[[i]]  <- cut(x, breaks = midpoint,
                    include.lowest = TRUE, labels = FALSE)

  list(cuts = cutpoint, disc = disc)
}

# ── Fast chiM on all features — replicates chiM()$Disc.data ──────────────────
fast_chiM_all <- function(data, alpha) {
  p <- ncol(data) - 1L
  for (i in seq_len(p)) {
    if (!is.numeric(data[[i]])) next
    val    <- fast_value(i, data, alpha)
    data   <- val$disc
  }
  data
}
...
# =============================================================================
# chi2_fast — matches chi2() output exactly
# =============================================================================
chi2_fast <- function(data, alp = 0.5, del = 0.05) {
  dat       <- data
  p         <- ncol(dat) - 1L
  alpha     <- alp
  delta     <- del
  d_outer   <- 0.1    # outer loop step — matches package: d <- 0.1

  # outer loop — matches package exactly:
  # inconRate of RAW data first, then chiM, then decrease alpha
  inconRate <- fast_incon(dat)
  Disc      <- fast_chiM_all(dat, alpha)

  while (inconRate < delta) {
    if (alpha <= 0.2) break
    alpha     <- alpha - d_outer
    Disc      <- fast_chiM_all(Disc, alpha)
    inconRate <- fast_incon(Disc)
  }

  # per-feature loop — matches package exactly
  eps    <- 0.01
  d_inner <- 0.05   # inner loop step — matches package: d <- 0.05
  sig    <- rep(alpha, p)
  cutp   <- vector("list", p)
  ...
  for (i in seq_len(p)) {
    ...
    val <- NULL
    repeat {
      val       <- fast_value(i, Disc, alpha = sig[i])
      Disc      <- val$disc
      inconRate <- fast_incon(Disc)
      sig[i]    <- sig[i] - d_inner
      if (inconRate > delta || sig[i] <= eps) break
    }
    cutp[[i]] <- if (is.null(val)) "" else val$cuts
  }
...
# =============================================================================
# fast_Xi() — faithful rewrite of Xi() from discretization package
# Rough-set class evidence degree consistency measure
# =============================================================================
fast_Xi <- function(data) {
  ...
  # group rows by identical feature pattern — same logic as Xi()
  keys   <- do.call(paste, c(lapply(feat, as.character), sep = "\r"))
  groups <- split(seq_len(n), keys)

  # only keep groups with duplicates
  dup_groups <- Filter(function(g) length(g) > 1L, groups)
  if (length(dup_groups) == 0L) return(0)

  # representative row index per group (first occurrence) — matches firs <- ww[1]
  rep_idx <- vapply(dup_groups, `[`, integer(1), 1L)

  # build freq table: rows = groups, cols = classes
  # matches freq = table(fnum, data[,p]) in Xi()
  all_classes <- sort(unique(y))
  freq <- matrix(0L, nrow = length(dup_groups), ncol = K)
  for (gi in seq_along(dup_groups)) {
    class_vals <- y[dup_groups[[gi]]]
    for (k in seq_len(K))
      freq[gi, k] <- sum(class_vals == all_classes[k])
  }

  # CED per class — matches Xi() inner loop over j
  CED <- vapply(seq_len(K), function(j) {
    # Ctable = rows where class j count > 0
    rows_with_j <- freq[freq[, j] > 0L, , drop = FALSE]
    if (nrow(rows_with_j) == 0L) return(NA_real_)

    Csum     <- sum(rows_with_j)
    rSum     <- rowSums(rows_with_j)
    rMax     <- apply(rows_with_j, 1, max)
    misClass <- sum(rSum - rMax)
    1 - misClass / Csum
  }, numeric(1))

  CED <- CED[!is.na(CED)]
  if (length(CED) == 0L) return(0)

  # Xi = max(m1, m2) — matches Xi() formula exactly
  m1 <- if (any(CED >= 0.5)) 1 - min(CED[CED >= 0.5]) else 0
  m2 <- if (any(CED <= 0.5)) max(CED[CED <= 0.5])     else 0
  max(m1, m2)
}

# =============================================================================
# extendChi2_fast() — faithful rewrite of extendChi2() from package
# =============================================================================
extendChi2_fast <- function(data, alp = 0.5) {
  dat   <- data
  p     <- ncol(dat) - 1L
  alpha <- alp
  d_out <- 0.1     # outer step — matches d = 0.1
  e     <- 0.01    # outer threshold — matches e <- 0.01
  d_in  <- 10      # inner divisor — matches d <- 10
  eps   <- 1e-4    # inner stop — matches eps <- 1e-04

  # outer loop — matches extendChi2() exactly
  # XiO = Xi of RAW data, run chiM once, then iterate
  XiO  <- fast_Xi(dat)
  Disc <- fast_chiM_all(dat, alpha)
  XiN  <- fast_Xi(Disc)
  ...
  while ((XiN - XiO) < e) {
    if (alpha <= 0.2) break
    alpha <- alpha - d_out
    Disc  <- fast_chiM_all(Disc, alpha)   # chiM on already-discretised
    XiN   <- fast_Xi(Disc)
    XiO   <- XiN                          # update XiO = XiN each step
  }

  # per-feature loop — matches extendChi2() exactly
  sig  <- rep(alpha, p)
  cutp <- vector("list", p)
  ...
  for (i in seq_len(p)) {
    ...
    XiO <- fast_Xi(Disc)    # Xi before this feature's refinement
    val <- NULL
    ...
    repeat {
      ...
      Disc      <- val$disc
      XiN       <- fast_Xi(Disc)
      XiO       <- XiN           # immediately overwrite — matches package
      sig[i]    <- sig[i] / d_in # divide by 10 each step
...
# =============================================================================
# fast_topdown_cacc() — faithful rewrite of disc.Topdown(method=2)
# Matches package output exactly
# =============================================================================

# ── chiSq with 1e-4 smoothing — matches chiSq() exactly ──────────────────────
fast_chiSq <- function(tb) {
  tb <- tb + 1e-4
  n  <- sum(tb)
  mi <- rowSums(tb);  ni <- colSums(tb)
  E  <- outer(mi, ni) / n
  sum((tb - E)^2 / E)
}

# ── CACC criterion — matches cacc() exactly ───────────────────────────────────
fast_cacc_crit <- function(tb) {
  n    <- sum(tb)
  nr   <- nrow(tb) + 1L
  logn <- if (nr > 1L) log(nr) else 0
  yp   <- fast_chiSq(tb)
  sqrt(yp / (yp + n * logn))
}

# ── single-feature top-down CACC ──────────────────────────────────────────────
topdown_cacc_feature <- function(xo, yo, K, n) {
  ci <- which(diff(xo) != 0L)
  if (length(ci) == 0L) return(c(xo[1L], xo[n]))

  cuts    <- (xo[ci] + xo[ci + 1L]) / 2
  bd      <- c(xo[1L], xo[n])
  di      <- cuts
  GlobalC <- 0
  k       <- 1L
  ...
  while (length(di) > 0L) {
    ...
    for (i in seq_along(di)) {
      bd1 <- sort(c(bd, di[i]))
      m   <- length(bd1) - 1L
      dff <- findInterval(xo, bd1, rightmost.closed = TRUE)
      tb  <- matrix(tabulate(dff + m * (yo - 1L), nbins = m * K),
                    nrow = m, ncol = K)
      cv  <- fast_cacc_crit(tb)
      if (cv > bestC) { bestC <- cv;  iBest <- i;  addCut <- di[i] }
    }

    if (!is.na(iBest)) di <- di[-iBest]

    if ((bestC > GlobalC || k < K) && k < n) {
      bd      <- sort(c(bd, addCut))
      GlobalC <- bestC
      k       <- k + 1L
    } else {
...
# ── full dataset wrapper — matches disc.Topdown(method=2) exactly ─────────────
fast_topdown_cacc <- function(data) {
  p1      <- ncol(data);  p <- p1 - 1L
  y       <- as.integer(as.factor(data[[p1]]))
  K       <- length(unique(y));  n <- nrow(data)
  cutList <- vector("list", p);  xd <- data

  for (i in seq_len(p)) {
    x <- data[[i]];  if (!is.numeric(x)) next
    od <- order(x);  xo <- x[od];  yo <- y[od]
    bd           <- topdown_cacc_feature(xo, yo, K, n)
    cutList[[i]] <- bd
    xd[[i]]      <- as.integer(findInterval(x, bd, rightmost.closed = TRUE))
  }
...
lr_predict <- function(fit, Disc.data) {
  as.character(predict(fit$model, newdata=Disc.data))
}

lr_predict_prob <- function(fit, Disc.data) {
  prob <- tryCatch(predict(fit$model, newdata=Disc.data, type="probs"),
...
xgb_train <- function(Disc.data) {
  ...
  params <- Filter(Negate(is.null), list(
    objective   = if (n_cls==2) "binary:logistic" else "multi:softprob",  # <── fix
...
xgb_predict <- function(fit, Disc.data) {
  ...
  if (fit$n_cls == 2L) {
    # binary:logistic → single probability per row
    idx <- ifelse(raw > 0.5, 2L, 1L)
  } else {
    # multi:softprob → flat vector of K probs per row, take argmax
    prob_mat <- matrix(raw, ncol=fit$n_cls, byrow=TRUE)
    idx      <- max.col(prob_mat)
  }
  fit$classes[idx]
}
...
compute_auc <- function(prob_mat, labels) {
  ...
  if (is.null(prob_mat) || any(is.na(prob_mat))) return(NA_real_)

  # Ensure we have more than one class in the labels
  if (length(unique(labels)) < 2) return(NA_real_)

  # Check for zero variance: if the model predicts the same thing for everyone,
  # AUC is mathematically 0.5, but pROC might throw an error.
  if (nrow(unique(round(prob_mat, 10))) <= 1) return(0.5)

  if (!has_pkg("pROC")) {
    message("pROC not installed.")
    return(NA_real_)
  }
  ...
  tryCatch({
    ...
    # Ensure AUC is reported on the 0.5 - 1.0 scale
    #if (!is.na(auc) && auc < 0.5) auc <- 1 - auc
...
cv_run <- function(data, disc_fn, clf_types, k=10, seed=42,
  ...
  for (fold in seq_len(k)) {
    ...
    for (ct in clf_types) {
      ...
      if (!is.null(preds))
        correct[ct] <- correct[ct] + sum(preds == y_test, na.rm=TRUE)
      ...
      if (!is.null(probs)) {
        ...
        # strip NA rows before accumulating (e.g. LR with unseen bins)
        ok <- complete.cases(aligned)
        if (any(ok)) {
          fold_probs[[ct]]$probs  <- rbind(fold_probs[[ct]]$probs, aligned[ok, , drop=FALSE])
          fold_probs[[ct]]$labels <- c(fold_probs[[ct]]$labels, y_test[ok])
        }
      }
    }
  }
...
build_disc_methods <- function(lambda=2) {
  ...
  if (has_pkg("discretization")) {
    ...
    methods[["CACC"]]       <- function(d, ...)  fast_topdown_cacc(d)
    ...
    methods[["Chi2"]]       <- function(d, ...) chi2_fast(d)
    methods[["ExtendChi2"]] <- function(d, ...) extendChi2_fast(d,alp   = 0.5)
...
# =============================================================================
# PART 6 — Dataset loader (22 datasets)
#
# Datasets : Iris, BreastCancer, Ionosphere, Glass, Vehicle,
#            PimaDiabetes, Vowel, Satellite, Shuttle, BostonHousing, Ozone,
#            Waveform, Normals2D, Circle, Spirals, Cassini,
#            Ringnorm, Threenorm, Twonorm, AirQuality
#
...
run_comparison2 <- function(datasets, lambda=2, k=10, seed=42) {
  ...
  for (ds_name in names(datasets)) {
    ...
    res_disc <- tryCatch(
      ...
      error=function(e){cat(" ERROR:",conditionMessage(e),"\n");NULL})
    if (!is.null(res_disc))
      nb_auc_str <- if(is.na(res_disc$auc["nb"])) "NA" else sprintf("%.3f", res_disc$auc["nb"])
    cat(sprintf("  NB=%.1f%%/AUC=%s", res_disc$accuracy["nb"]*100, nb_auc_str),
        if(length(shared_clf))
          paste(mapply(function(ct, lbl) {
            a <- res_disc$auc[ct]
            sprintf("  %s=%.1f%%/AUC=%s", lbl, res_disc$accuracy[ct]*100,
                    if(is.na(a)) "NA" else sprintf("%.3f", a))
          }, shared_clf, clf_labels[shared_clf]), collapse=""),
        "\n", sep="")
...
loaded   <- load_datasets(n_shuttle = 10000)
