...
make_table1 <- function(datasets, meta) {
  meta_lookup <- data.frame(
    Dataset = c("Iris","BreastCancer","Ionosphere","Glass","Vehicle","Sonar",
                ...
                "Waveform","Normals2D","Circle","Spirals","Cassini",
                "Ringnorm","Threenorm","Twonorm","AirQuality"),
    Source  = c(rep("mlbench", 12), rep("mlbench", 8), rep("base R", 1)),
    Type    = c(rep("Real", 12), rep("Synthetic", 8), rep("Real", 1)),
...
run_comparison1 <- function(datasets, disc_methods,
  ...
  list(results=do.call(rbind, rows), acc_matrix=acc_matrix)
}
...
run_comparison2 <- function(datasets, lambda=2, k=10, seed=42) {
  ...
  cat("\n\n",rep("=",65),"\n",sep="")
  cat(" COMPARISON 2 — GOLD vs None (accuracy only)\n")
  cat(rep("=",65),"\n",sep="")
...
make_table5 <- function(bench1) {
  ...
  cat("\n=== Table 5: Post-hoc Wilcoxon (GOLD vs each discretiser) ===\n")
  print(t5, row.names=FALSE);  write.csv(t5,"table5_posthoc.csv",row.names=FALSE)
  invisible(stat1)
}
...
make_table6 <- function(bench2) {
  ...
  rows <- lapply(pairs, function(pr) {
    if (!pr$d %in% names(bench2) || !pr$n %in% names(bench2)) return(NULL)
    d <- as.numeric(bench2[[pr$d]]);  n <- as.numeric(bench2[[pr$n]])
    ok <- complete.cases(data.frame(d,n));  if(sum(ok)<5) return(NULL)
    wt <- tryCatch(wilcox.test(d[ok],n[ok],paired=TRUE,exact=FALSE),error=function(e)NULL)
    if(is.null(wt)) return(NULL)
    data.frame(Classifier_Pair=pr$label, N=sum(ok),
  ...
  t6 <- do.call(rbind, Filter(Negate(is.null), rows));  rownames(t6) <- NULL
  cat("\n=== Table 6: Wilcoxon — GOLD vs None per classifier ===\n")
  print(t6, row.names=FALSE);  write.csv(t6,"table6_wilcoxon_none.csv",row.names=FALSE)
  invisible(t6)
}
...
bench1 <- run_comparison1(datasets['Vowel'], disc_methods, k=10, seed=42)
