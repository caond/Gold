...
#   Statistics  : Friedman test + Holm-corrected Wilcoxon (Gold vs each)
#
...
# =============================================================================
# PART 1 — Gold discretisation
# =============================================================================

Gold_discretize_logistic <- function(x, y, min_bin_size = 5, lambda = 1) {
...
Gold <- function(data, lambda = 1) {
  ...
  for (i in feature_cols) {
    ...
    res        <- Gold_discretize_logistic(col, data[[class_col]],
...
build_disc_methods <- function(lambda = 1) {
  ...
  if (has_pkg("discretization")) {
    ...
    message("'discretization' not installed — only Gold will run in Comparison 1.")
  }
  methods
}
...
# =============================================================================
# PART 6 — COMPARISON 1: discretisers vs Gold
...
run_comparison1 <- function(datasets, disc_methods,
  ...
  cat("\n", rep("=",65), "\n", sep="")
  cat(" COMPARISON 1 — Discretisers vs Gold\n")
  cat(rep("=",65), "\n", sep="")
...
# =============================================================================
# PART 7 — COMPARISON 2: Gold vs None — accuracy only
...
run_comparison2 <- function(datasets, lambda = 2,
  ...
  # per-classifier: Gold accuracy and None accuracy across datasets
  ...
  for (ds_name in names(datasets)) {
    ...
    # ---- Gold on discretised data (nb + shared classifiers) --------------
...
stat_test_comparison1 <- function(acc_matrix) {
  ...
  cat("(positive Mean_Diff = Gold wins)\n\n")
...
print_summary2 <- function(comp2_results) {
  cat("\n\n", rep("=",65), "\n", sep="")
  cat(" Comparison 2 Summary — Gold vs None (mean accuracy diff)\n")
  cat(rep("=",65), "\n\n", sep="")
  ...
  cat("\n  (positive Mean_Diff = discretised Gold outperforms raw data)\n")
}
...
save_d   <- datasets
datasets <-list()
datasets[["Twonorm"]]<-save_d$Twonorm
...
# ── Comparison 2: Gold vs None (accuracy only) ────────────────────────────
